...
import type { InfiniteMenuItem } from '../types/infiniteMenu'
...
const emit = defineEmits<{
  activeItemChange: [item: InfiniteMenuItem | null]
  movementChange: [isMoving: boolean]
}>()
...
class DiscGeometry extends Geometry {
  constructor(steps = 4, radius = 1) {
    ...
    const centerVertex = this.lastVertex
    if (centerVertex) {
      centerVertex.uv[0] = 0.5
      centerVertex.uv[1] = 0.5
    }
    ...
    for (let i = 0; i < steps; ++i) {
      ...
      const lastVertex = this.lastVertex
      if (lastVertex) {
        lastVertex.uv[0] = x * 0.5 + 0.5
        lastVertex.uv[1] = y * 0.5 + 0.5
      }
...
class InfiniteGridMenu {
  ...
  constructor(
    ...
    // discBuffers will be initialized in #init
    this.discBuffers = {
  ...
  #init(onInit: ((sketch: InfiniteGridMenu) => void) | null) {
    ...
    if (!this.discProgram) throw new Error('Failed to create program')
    ...
    this.discBuffers = {
      ...
      normals: this.discGeo.normalData,
    ...
    if (!vertexBuffer || !uvBuffer) throw new Error('Failed to create buffers')
  ...
  #animate(deltaTime: number) {
    ...
    positions.forEach((p, ndx) => {
      ...
      if (instanceMatrix) {
        mat4.copy(instanceMatrix, matrix)
      }
    })
  ...
  #render() {
    const gl = this.gl
    if (!gl || !this.discProgram || !this.discVAO) return
    ...
    if (this.discLocations.uWorldMatrix)
      gl.uniformMatrix4fv(this.discLocations.uWorldMatrix, false, this.worldMatrix)
    if (this.discLocations.uViewMatrix)
      gl.uniformMatrix4fv(this.discLocations.uViewMatrix, false, this.camera.matrices.view)
    if (this.discLocations.uProjectionMatrix)
      gl.uniformMatrix4fv(
    ...
    if (this.discLocations.uRotationAxisVelocity)
      gl.uniform4f(
    ...
    if (this.discLocations.uItemCount)
  ...
  #onControlUpdate(deltaTime: number) {
    ...
    if (!this.control.isPointerDown) {
      ...
      const worldPos = this.#getVertexWorldPosition(nearestVertexIndex)
      if (worldPos) {
        const snapDirection = vec3.normalize(vec3.create(), worldPos)
  ...
  #getVertexWorldPosition(index: number): vec3 | null {
    const nearestVertexPos = this.instancePositions[index]
    if (!nearestVertexPos) return null
...
const handleActiveItem = (index: number) => {
  ...
  activeItem.value = props.items[itemIndex] || null
...
// Note: Items should be set before mounting. Dynamic updates would require exposing initTexture method.
</script>

<template>
  <div class="infinite-menu-container">
    <canvas ref="canvasRef" id="infinite-grid-menu-canvas" class="infinite-menu-canvas" />

    <div v-if="activeItem" class="menu-overlay">
      ...
      >
        <span class="material-symbols-outlined action-button-icon">arrow_outward</span>
      </button>
    </div>
  </div>
</template>
